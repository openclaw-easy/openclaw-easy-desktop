...
// Openclaw Easy fork (UPSTREAM_FORK_PATCHES P7): the fork's own desktop release
// workflows are not upstream's to police. They build two pnpm workspaces in one
// job — the root at packageManager's pnpm, and moltbot-easy at pnpm 8 — so they
// pin pnpm explicitly, which upstream's packageManager-only rule forbids.
const FORK_OWNED_WORKFLOWS = new Set(["build-macos.yml", "build-windows-desktop.yml"]);

function workflowPaths(): string[] {
  return readdirSync(".github/workflows")
    .filter((name) => name.endsWith(".yml") && !FORK_OWNED_WORKFLOWS.has(name))
