...
function listNormalFullSuiteTestFiles(): string[] {
  ...
  return globSync(["**/*.{test,spec}.{ts,tsx,mts,cts,js,jsx,mjs,cjs}"], {
    cwd: process.cwd(),
    // Openclaw Easy fork (UPSTREAM_FORK_PATCHES P4): `moltbot-easy/` is the fork's
    // own workspace — its desktop/backend suites have their own runners, and its
    // build output (`dist-installers/`, `installer-resources/`) embeds a full copy
    // of the openclaw tree that this repo-wide glob would otherwise claim.
    exclude: [
      "**/.*/**",
      "**/dist/**",
      "**/node_modules/**",
