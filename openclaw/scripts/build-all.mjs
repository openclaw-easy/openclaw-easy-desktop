...
export const BUILD_ALL_STEPS = [
  ...
  // Control UI (Vite project at ui/, output to dist/control-ui/). MUST run
  // after tsdown — tsdown.config.ts has `clean: true` which wipes the entire
  // dist/ tree at the start of a build, so anything written before tsdown
  // would be deleted. Without this step, the gateway's "Open Web UI" button
  // 404s with "Control UI assets not found" after every `pnpm build`. The
  // desktop's predev/prebuild hooks build the UI separately, but any
  // subsequent `pnpm build` (e.g. from openclaw's own dev loop or a
  // contributor running build at the upstream root) would clear it again —
  // this step guarantees the UI is always present whenever dist/ is.
  // Cache: keyed on ui/ source + the tsdown output it co-exists with so a
  // partial dist (e.g. tsdown only) re-triggers a UI build.
  {
    label: "ui:build",
    kind: "node",
    args: ["scripts/ui.js", "build"],
    cache: {
      inputs: ["ui/src", "ui/index.html", "ui/vite.config.ts", "ui/package.json"],
      outputs: ["dist/control-ui/index.html"],
    },
  },
