...
function makeTempDir() {
  // Canonicalize: on macOS os.tmpdir() is a /var -> /private/var symlink, and
  // production resolvers return realpaths. Comparing a raw mkdtemp path against
  // a resolver result passes on Linux CI and fails on macOS.
  return fs.realpathSync(makeTrackedTempDir("openclaw-plugin-registry-snapshot", tempDirs));
}
