...
describe("TypeScript sibling extension fallback", () => {
  // Internal-core aliases point bare @openclaw/* specifiers at packages/<pkg>/src/*.ts.
  // Those sources import siblings by emitted name ("./base64.js"), which exists only
  // as .ts on disk. Without this fallback the first relative import inside an aliased
  // package throws ERR_MODULE_NOT_FOUND — that is how the 2026-07 upstream sync broke
  // `pnpm run build` at the write-cli-startup-metadata step.
  function makeTsPackage(prefix: string): string {
    return fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), prefix)));
  }

  it("resolves an emitted-name sibling import from a .ts parent to the .ts source", () => {
    const root = makeTsPackage("openclaw-ts-sibling-");
    fs.writeFileSync(path.join(root, "base64.ts"), "export const value = 'ts';\n", "utf8");
    const parent = path.join(root, "inline-image-data-url.ts");
    fs.writeFileSync(parent, "export { value } from './base64.js';\n", "utf8");

    installOpenClawPluginSdkNativeResolver({});

    const requireFromParent = createRequire(parent);
    expect(fs.realpathSync(requireFromParent.resolve("./base64.js"))).toBe(
      fs.realpathSync(path.join(root, "base64.ts")),
    );
  });

  it("does not shadow a real emitted .js sibling", () => {
    const root = makeTsPackage("openclaw-ts-sibling-emitted-");
    fs.writeFileSync(path.join(root, "base64.js"), "module.exports = { value: 'js' };\n", "utf8");
    fs.writeFileSync(path.join(root, "base64.ts"), "export const value = 'ts';\n", "utf8");
    const parent = path.join(root, "inline-image-data-url.ts");
    fs.writeFileSync(parent, "export { value } from './base64.js';\n", "utf8");
    ...
    const requireFromParent = createRequire(parent);
    expect(fs.realpathSync(requireFromParent.resolve("./base64.js"))).toBe(
      fs.realpathSync(path.join(root, "base64.js")),
    );
  });

  it("leaves bare specifiers and non-TypeScript parents alone", () => {
    const root = makeTsPackage("openclaw-ts-sibling-inert-");
    fs.writeFileSync(path.join(root, "sibling.ts"), "export const value = 'ts';\n", "utf8");
    const jsParent = path.join(root, "parent.js");
    fs.writeFileSync(jsParent, "module.exports = {};\n", "utf8");

    installOpenClawPluginSdkNativeResolver({});

    // A .js parent must not get the TypeScript fallback.
    const requireFromJsParent = createRequire(jsParent);
    expect(() => requireFromJsParent.resolve("./sibling.js")).toThrow();
  });
});
