...
describe("copyBundledPluginMetadata", () => {
  ...
  it.each([
    ...
    {
      // Openclaw Easy fork (UPSTREAM_FORK_PATCHES P2): whatsapp stays in the
      // core bundle, so it is no longer an externalized plugin here. qqbot is
      // the only remaining EXCLUDED_CORE_BUNDLED_PLUGIN_DIRS entry, so it is
      // what exercises this path in this fork.
      name: "removes externalized optional plugin metadata from the core dist",
      pluginId: "qqbot",
      packageName: "@openclaw/qqbot",
      packageOpenClaw: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@openclaw/qqbot" },
      },
      env: {},
      expectedExists: false,
    },
