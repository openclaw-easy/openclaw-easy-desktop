...
function resolveTypeScriptSiblingPath(
  ...
  return pluginCacheExistsSync(candidate) ? candidate : undefined;
}
...
function installResolver(): void {
  ...
  moduleWithResolver[nodeResolveFilenameProperty] = ((request, parent, isMain, options) =>
    ...
    resolveTypeScriptSiblingPath(request, parent?.filename) ??
  ...
  moduleWithResolver.registerHooks?.({
    resolve(specifier, context, nextResolve) {
      ...
      // An aliased `.ts` parent importing `./sibling.js` has no such file on disk,
      // and nextResolve throws on it, so this fallback has to precede that call.
      const resolvedTarget =
        aliasTarget ??
        resolveTypeScriptSiblingTargetForParentUrl(specifier, context.parentURL);
      const resolved = resolvedTarget
        ? { shortCircuit: true, url: pathToFileURL(resolvedTarget).href }
